...
class MC_CrossEntropy(nn.Module):
    ...
    def forward(
        ...
        noisy_pred = (
            ...
            + torch.sqrt(torch.exp(pred_log_var)).unsqueeze(0) * noise
        )
        del noise
        noisy_pred = noisy_pred.view(-1, *pred_mean.shape[1:])
        tiled_target = (
            target.unsqueeze(0).tile((self.mc_samples,)).view(-1, *target.shape[1:])
        )
...
def average_surface_distance(preds, target, include_background=True):
    ...
    return MM.compute_average_surface_distance(
        binairy_preds, binairy_target, include_background=include_background
    )
