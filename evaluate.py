...
def setup(args):
    ...
    checkpoint_path = (
        retrieve_checkpoint(model_id=args.model_id, project=args.project)
    ...
    if args.dataset == "tecfideramri":
        ...
        dataset = pl_loader._make_dataset("test", data_files=pl_loader.file_split)
    elif args.dataset == "skmtea":
        dataset = pl_loader._make_dataset("test", pl_loader.file_split)
...
class Evaluate:
    ...
    def proces_recon(self, batch, idx):
        ...
        if self.args.mc_samples > 1:
            uncertainty = torch.abs(pred[-1]).clone()
    ...
    def proces_srs(self, batch, idx):
        ...
        if self.args.mc_samples > 1:
            uncertainty = torch.abs(pred_recon[-1]).clone()
            self.tmp_data_dict[fname]["aleatoric_uncertainty"].append(
                (uncertainty / uncertainty.amax((-1, -2), True)).squeeze().cpu().numpy()
            )
            self.tmp_data_dict[fname]["epistemic_uncertainty"].append(
        ...
        if (idx + 1) % self.args.mc_samples == 0:
            ...
            recon = torch.abs(pred_recon[-1]).clone()
            recon = (recon / recon.amax((-1, -2), True)).squeeze()
            gt = batch[4].clone()
            gt = (gt / gt.amax((-1, -2), True)).squeeze()
    ...
    def save_h5(self, data_dict, metric_dict, key=None):
        if key in data_dict.keys():
            ...
            for fname, recons in data_dict.items():
                save_name = fname
                if fname.split(".")[-1] != "h5":
                    save_name = f"{save_name.split('.')[0]}.h5"
                with h5py.File(os.path.join(self.args.out_dir, save_name), "w") as hf:
    ...
    def get_metrics(self):
        ...
        for key, value in results.items():
            print(f"{key}: \t {np.mean(np.ma.masked_invalid(np.array(value)))}")
        print("---------")
        return results
